...
function handleDisconnect() {
    dbPool.getConnection((err, connection) => {
        if (err) {
            ...
            // Jika koneksi terputus karena PROTOCOL_CONNECTION_LOST, lakukan reconnect
    ...
    // Tangani event error pada pool
    dbPool.on('error', (err) => {
        console.error('Database pool error:', err);

        // Tangani jika terjadi error PROTOCOL_CONNECTION_LOST
        if (err.code === 'PROTOCOL_CONNECTION_LOST') {
            console.log('Koneksi database terputus. Mencoba untuk reconnect...');
            handleDisconnect();
        } else {
...
async function connectToWhatsApp() {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            // Notify the client that WhatsApp is connected
            io.emit('connection.update', { status: 'connected' });
        }

        if (qr) {
            qrcodeTerminal.generate(qr, { small: true }); // Menampilkan QR di terminal dalam ukuran kecil
            try {
                ...
                fs.writeFile('qr_code.json', JSON.stringify({ qr: qrBase64 }), (err) => {
                    if (err) {
                        ...
                        // Notify the client to update the QR code
    ...
    sock.ev.on('messages.upsert', async (msg) => {
        ...
        if (!message.key.fromMe && message.message) {
            ...
            // Cek dan daftar user menggunakan fungsi checkAndRegisterUser
